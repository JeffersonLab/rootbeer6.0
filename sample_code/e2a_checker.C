...
//                                                                            //
//                     Stuart Fegan (stuart.fegan@york.ac.uk)                 //
//                                                                            //
//      Version         Date                    Comment                       //
//      1                                       Version of rbtest looking at  //
//                                              beam position variables in e2a//
//                                                                            //
//******************************** e2a_checker.C *****************************//
...
class   TH1F *momentum;              // A Sample 1d hist array for SCR

class   TH2F *IPM2H01;            	// A Sample 2d histogram for tagr
class   TH2F *IPM2C24A;            	// A Sample 2d histogram for tagr
class   TH2F *IPM2C22A;            	// A Sample 2d histogram for tagr
//                                                                      *
// **********************************************************************
...
//                                                                      *
void e2a_checker(int, char *,char *); // main func. (same name as this macro)
//                                                                      *
...
int main(int argc,char **argv){		
  ...
  e2a_checker(nEventsTot,InFileName,OutFileName);	//call user function
  return(0);
}
#endif
...
void e2a_checker(int nEvents, char *file, char *outFileName){   // main user function
  ...
  int	evFlag=0;			//flag for string disply line
  ...
  char *mybanks[]={"EPIC","EVNT","HEAD","null"};	// make a list of the banks you want to use
  // ** DONT FORGET TO DEFINE THE BANKS YOU WANT RIGHT HERE
  //
...
//   }
   ...
   IPM2H01 = new TH2F("IPM2H01","current info 2h01",
 		       1000,0.0,10000.0,100,0,20); 	//make 2d hist

   IPM2C24A = new TH2F("IPM2C24A","current info 2c24a",
 		       1000,0.0,10000.0,100,0,20); 	//make 2d hist

   IPM2C22A = new TH2F("IPM2C22A","current info 2c22a",
 		       1000,0.0,10000.0,100,0,20); 	//make 2d hist
...
//       }   
	...
	getEpics("IPM2H01 ",&IPM_2H01);
	getEpics("IPM2C24A ",&IPM_2C24A);
	getEpics("IPM2C22A ",&IPM_2C22A);


	//std::cout << "IPM2C24A = " << IPM_2C24A << std::endl;
	//std::cout << "IPM2C24A.X = " << IPM_2H01 << std::endl;
	std::cout << "IPM2H01 = " << IPM_2H01 << std::endl;
	std::cout << "IPM2C24A = " << IPM_2C24A << std::endl;
	std::cout << "IPM2C22A = " << IPM_2C22A << std::endl <<std::endl;;

      }

	IPM2H01->Fill(eventTot/1000,IPM_2H01);
	IPM2C24A->Fill(eventTot/1000,IPM_2C24A);
	IPM2C22A->Fill(eventTot/1000,IPM_2C22A);

	   
      // End of user customized section ----------------------------

      eventTot++;
      if(eventTot%10000 == 0) fprintf(stdout,"done %d\n",eventTot);
...
int parseArgs(int argc,char **argv){
  ...
  case(2):
    ...
    return(-1);
    break;
  case(3):				// 3 args e2a_checker <infile> <outfile>
    ...
    break;
  case(4):				// 4 args must be "e2a_checker -N# <infile> <outfile>" 
...
void printUsage(){
  fprintf(stderr,"\nusage:\n\n");
  fprintf(stderr,"e2a_checker -h   Print this message\n\n");
  fprintf(stderr,"e2a_checker [-Nevents] <infile> <outfile.root> \n");
  fprintf(stderr,"e2a_checker [-Nevents] <-Lfilelist> <outfile.root> \n");
}
